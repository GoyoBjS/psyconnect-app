...
import {extractDateFromTimestamp, extractTimeFromTimestamp} from "../../../../utils/extractDateTimeFromTimestamp";
...
const QuickRegistrationScreen = () => {
    ...
    const [data, setData] = useState<QuickRegistrationDataType>(() => { return {} as QuickRegistrationDataType })
    const auth = getAuth();
    ...
    useEffect(() => {
            if (step === 3) {
                instertData(data).then(() =>{
                    setStep(1)
                    setData(() => { return {} as QuickRegistrationDataType });
  ...
  return (
    <View>
        <>
            {step === 1 && <FeelingsScreen data={data} setData={setData} step={step} setStep={setStep} />}
